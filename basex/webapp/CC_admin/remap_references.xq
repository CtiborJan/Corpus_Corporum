...
declare variable $old_cc_idno external:="";
declare variable $new_cc_idno external:="";
declare variable $all external:="";
declare variable $cc:accessibility external:="0";

if ($all='true') then
    cc:remap_all_references($old_cc_idno,$new_cc_idno)
else
    cc:remap_selected_references($references,$new_cc_idno)
